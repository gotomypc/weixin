  ...
  return function (req, res, next) {
    getMessage(req, function (err, result) {
      ...
      storage.get(openid, function (err, session) {
        ...
        req.weixin = message;
        req.wesession = session || {};
        var _end = res.end;
        res.end = function () {
          _end.apply(res, arguments);
          storage.set(openid, req.wesession);
        };
 ...
 *  .middleware();
 * ```
 * @param {String} opts 在微信平台填写的口令
 * @param {Function} handle 生成的回调函数，参见示例
 */
var middleware = function (opts, handle) {
  ...
  var storage = opts.storage || new connect.session.MemoryStore();
  ...
  if (arguments.length === 2) {
    if (handle instanceof Handler) {
      ...
      var handler = new Handler(token, handle);
      handler.storage = storage;
      return handler.middlewarify();
    }
  }
};
